...
const API = {
  ...
  async getDuaCategories() {
    ...
    if (cached) return cached;
    try {
      const res = await fetch(`${CONFIG.API.UMMAH}/duas/categories`);
      const json = await res.json();
  ...
  async getDuasByCategory(categoryId) {
    ...
    if (cached) return cached;
    try {
      const res = await fetch(`${CONFIG.API.UMMAH}/duas/category/${encodeURIComponent(categoryId)}`);
      const json = await res.json();
      const duas = json?.data?.duas || [];
