// 🌐 Cliente API — Aladhan + Al-Quran Cloud
...
const API = {
  ...
  async gregorianToHijri(date = new Date()) {
    ...
    if (hijri) Storage.set(cacheKey, hijri, CONFIG.CACHE_TTL * 7); // 1 semana
    return hijri;
  },
  ...
  async getSurahList() {
    ...
    const json = await res.json();
    const data = json.data || [];
    Storage.set('surah_list', data, CONFIG.CACHE_TTL * 30); // 30 días
    return data;
  },

  async getSurahWithTranslation(surahNumber, translation = 'es.cortes', audio = 'ar.alafasy') {
    const cacheKey = `surah_${surahNumber}_${translation}_${audio}`;
    const cached = Storage.get(cacheKey);
    if (cached) return cached;

    const editions = `quran-uthmani,${translation},${audio}`;
    ...
    return new Promise((resolve, reject) => {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          // Reverse geocoding con Nominatim (gratuito, sin key)
          try {
