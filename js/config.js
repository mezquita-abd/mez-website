...
const CONFIG = {
  ...
  API: {
    ...
    // ☁️ Proxy backend opcional (Cloudflare Worker). Si está configurado,
    // se usa como primera opción para /translate, /duas y /geocode (evita rate limits).
    // Ver backend/README.md. Dejar '' para modo directo (sin proxy).
    PROXY: '',
  },

  // Preferir dataset local vetado para du'as (recomendado en producción)
  // Cambia a false para usar UmmahAPI (unofficial, más categorías pero sin revisión teológica formal)
  USE_LOCAL_DUAS: true,
